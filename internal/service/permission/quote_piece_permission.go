...
// GetQuotePiecePermission get QuotePiece permission
func GetQuotePiecePermission(ctx context.Context, userID string, creatorUserID string, status int,
	...
	if (canEdit || userID == creatorUserID) && status != entity.QuotePieceStatusDeleted {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "edit",
	...
	if canClose && status == entity.QuotePieceStatusAvailable {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "close",
	...
	if (canDelete || userID == creatorUserID) && status != entity.QuotePieceStatusDeleted {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "delete",
	...
	if canRecover && status == entity.QuotePieceStatusDeleted {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "undelete",
...
// GetQuotePieceExtendsPermission get QuotePiece extends permission
func GetQuotePieceExtendsPermission(ctx context.Context, canInviteOtherToAnswer bool) (
	actions []*schema.PermissionMemberAction) {
	lang := handler.GetLangByCtx(ctx)
	actions = make([]*schema.PermissionMemberAction, 0)
