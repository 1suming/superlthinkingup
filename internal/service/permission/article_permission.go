...
// GetArticlePermission get question permission
func GetArticlePermission(ctx context.Context, userID string, creatorUserID string, status int,
	...
	if (canEdit || userID == creatorUserID) && status != entity.ArticleStatusDeleted {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "edit",
	...
	if canClose && status == entity.ArticleStatusAvailable {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "close",
	...
	if (canDelete || userID == creatorUserID) && status != entity.ArticleStatusDeleted {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "delete",
	...
	if canRecover && status == entity.ArticleStatusDeleted {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "undelete",
...
// GetArticleExtendsPermission get question extends permission
func GetArticleExtendsPermission(ctx context.Context, canInviteOtherToAnswer bool) (
	actions []*schema.PermissionMemberAction) {
	lang := handler.GetLangByCtx(ctx)
	actions = make([]*schema.PermissionMemberAction, 0)
