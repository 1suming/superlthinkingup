...
// GetQuotePermission get question permission
func GetQuotePermission(ctx context.Context, userID string, creatorUserID string, status int,
	...
	if (canEdit || userID == creatorUserID) && status != entity.QuoteStatusDeleted {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "edit",
	...
	if canClose && status == entity.QuoteStatusAvailable {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "close",
	...
	if (canDelete || userID == creatorUserID) && status != entity.QuoteStatusDeleted {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "delete",
	...
	if canRecover && status == entity.QuoteStatusDeleted {
		actions = append(actions, &schema.PermissionMemberAction{
			Action: "undelete",
...
// GetQuoteExtendsPermission get question extends permission
func GetQuoteExtendsPermission(ctx context.Context, canInviteOtherToAnswer bool) (
	actions []*schema.PermissionMemberAction) {
	lang := handler.GetLangByCtx(ctx)
	actions = make([]*schema.PermissionMemberAction, 0)
